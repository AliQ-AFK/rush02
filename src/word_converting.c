...
char* find_in_dict(char *num_str, DictEntry dictionary[], int size)
{
    ...
    while (i < size)
    {
        ...
        {
            return dictionary[i].value; // Return the dictionary value as is
        }
        i++;
    }
    return NULL;
...
void convert_to_words(char *num_str, DictEntry dictionary[], int size) {
    ...
    while (len > 0) {
        ...
        if (group_count > 0 && ft_strlen(output) > 0) {
            if (suffixes[group_count][0] != '\0') {
                ft_strcat(output, " ");
    ...
    int i = group_count - 1;
    int first_word = 1;  // Flag to handle spacing between groups
    while (i >= 0)
    {
        if (ft_strlen(groups[i]) > 0) {
            if (!first_word) {
                ft_putstr(" ");  // Add space before each group
            }
            ft_putstr(groups[i]);
            first_word = 0;
        }
